...
public class PagedList<T> : List<T>
{
    ...
    public PagedList(List<T> items, int count, int pageNumber, int limit)
    {
        TotalCount = count;
        Limit = limit;
    ...
    public static async Task<PagedList<T>> CreateAsync(IQueryable<T> query, int pageNumber, int limit)
    {
        var count = await query.CountAsync(); // Đếm tổng số
        var items = await query.Skip((pageNumber - 1) * limit).Take(limit).ToListAsync(); // Bốc dữ liệu
